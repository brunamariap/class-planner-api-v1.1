...
from person.api.serializers import UserSerializer
from person.api.permissions import IsTeacher, IsAdmin
...
class TeacherViewSet(ModelViewSet):
    ...
    def get_teacher_week_schedules(self, request, teacher_id):
        ...
        disciplines = Discipline.objects.filter(id__in=teach.values_list(
            'discipline_id', flat=True)).values_list('id', flat=True)
        classes = Class.objects.filter(id__in=teach.values_list(
            'class_id', flat=True)).values_list('id', flat=True)
        schedules = Schedule.objects.filter(
            class_id__in=classes, discipline_id__in=disciplines)
    ...
    def get_teacher_replace_classes(self, request, teacher_id):
        ...
        replace_classes = ClassCanceledSerializer(
            canceled_classes, many=True, context={'request': request})
...
class TeacherDisciplinesViewSet(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            serializer = TeacherDisciplineSerializer(
                teach, many=True, context={'teacher_id': teacher_id})
