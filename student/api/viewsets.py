...
class StudentViewSet(ModelViewSet):
    ...
    def create(self, request):
        ...
        #     registration=
        # )
        # É preciso criar um usuário antes de ter um estudante, até porque ele também será um usuário
        ...
        student_class = Class.objects.get(
            course_id=course, reference_period=period, shift=student_class_shift)
    ...
    @action(
        methods=['GET'],
        detail=False, url_path='byregistration/(?P<student_registration>[^/.]+)',
    ...
    def get_week_schedules(self, request, student_id):
        ...
        student_disciplines = Discipline.objects.filter(
            code__in=student_values.data['disciplines']).values_list('code', flat=True)
        ...
        schedules = Schedule.objects.filter(class_id__in=classes.values_list(
            'id', flat=True), discipline_id__in=disciplines.values_list("id", flat=True))
                ...
                if result.weekday() == current_schedule.weekday:
                    try:
                        wasCanceled = ClassCanceled.objects.get(
                            schedule_id=current_schedule.id, canceled_date=result)
        ...
        serializer = ScheduleSerializer(week_schedules, many=True, context={
    ...
    def get_month_schedules(self, request, student_id):
        ...
        serializer = ScheduleSerializer(month_schedules, many=True, context={
                                        'request': request, 'student_id': student_id})
...
class StudentAlertViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        teacher = User.objects.get(
            id=request.data['teacher_id'])  # Validar vínculo
