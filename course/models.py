...
class Discipline(models.Model):
    ...
    courses = models.ManyToManyField(
        Course,
        related_name="course_disciplines",
        through="CourseDiscipline",
        through_fields=("discipline_id", "course_id", "period"),
    )
...
class CourseDiscipline(models.Model):
    ...
    discipline_id = models.ForeignKey(
        Discipline, on_delete=models.CASCADE, db_column="discipline_id"
    )
...
class Class(models.Model):
    ...
    # Validar se é unico se tiver uma matrícula, se for nulo é nulo
    class_leader_id = models.CharField(
        max_length=20,
        null=True,
        blank=True,
        default=None,
...
class ClassCanceled(models.Model):
    ...
    schedule_id = models.ForeignKey(
        Schedule, on_delete=models.CASCADE, db_column="schedule_id"
    )
    canceled_date = models.DateField()
    reason = models.TextField(max_length=200, blank=True, null=True)
    replace_class_status = models.CharField(
        max_length=10, choices=Status.choices, default="Pendente", blank=True
    )
    canceled_by = models.IntegerField()
    teacher_to_replace = models.ForeignKey(
        User,
        db_column="teacher_to_replace",
        blank=True,
...
class Teach(models.Model):
    ...
    teacher_id = models.ForeignKey(
        User, on_delete=models.CASCADE, db_column="teacher_id"
    )
    discipline_id = models.ForeignKey(
        Discipline, on_delete=models.CASCADE, db_column="discipline_id"
    )
...
class TemporaryClass(models.Model):
    ...
    class_canceled_id = models.ForeignKey(
        ClassCanceled, on_delete=models.CASCADE, db_column="class_canceled_id"
    )
    teacher_id = models.ForeignKey(
        User, on_delete=models.CASCADE, db_column="teacher_id"
    )
